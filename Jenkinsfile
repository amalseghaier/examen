pipeline {
    ...
    // sh '${NODEJS_HOME}/bin/npm install bcrypt'
        ...
        stage('Build Docker image') {
            steps {
                sh 'docker build -t amalseghaier/examens:latest Dockerfile .'
                // Tag the Docker image with a version
                sh 'docker tag amalseghaier/examens:latest amalseghaier/examens:latest'
            }
        }
                ...
                script {
                    ...
                    withCredentials([string(credentialsId: 'token', variable: 'DOCKER_TOKEN')]) {
                        docker.withRegistry('https://index.docker.io/v1/', '12') {
                            ...
                            docker.image('amalseghaier/examens:latest').push('latest')
                        }
                    }
                }
            }
